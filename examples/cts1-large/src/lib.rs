...
mod page {
    use super::*;
    use constants::MAX_CONTRACT_STATE_SIZE;
    ...
    #[init(contract = "page", parameter = "ContractAddress", low_level)]
    fn contract_init(ctx: &impl HasInitContext, state: &mut ContractState) -> InitResult<()> {
        let address: ContractAddress = ctx.parameter_cursor().get()?;
        address.serial(state)?;
        Ok(())
    }

    #[receive(contract = "page", name = "read", parameter = "ReadParams", low_level)]
    fn contract_read<A: HasActions>(
        ctx: &impl HasReceiveContext,
        state: &mut ContractState,
    ) -> ReceiveResult<A> {
        let entry = ContractAddress::deserial(state)?;
        let sender = if let Address::Contract(address) = ctx.sender() {
            ensure_eq!(address, entry);
            address
        } else {
            bail!()
        };
        let params: ReadParams = ctx.parameter_cursor().get()?;
        state.seek(SeekFrom::Current(params.offset.into()))?;
        let mut data = Vec::with_capacity(params.length as usize);
        for _ in 0..params.length {
            data.push(state.read_u8()?)
        }
        Ok(A::send_raw(&sender, params.callback.as_ref(), Amount::from_micro_gtu(0), &data))
    }

    #[receive(contract = "page", name = "write", parameter = "WriteParams", low_level)]
    fn contract_write<A: HasActions>(
        ctx: &impl HasReceiveContext,
        state: &mut ContractState,
    ) -> ReceiveResult<A> {
        let entry = ContractAddress::deserial(state)?;
        if let Address::Contract(address) = ctx.sender() {
            ensure_eq!(address, entry);
        } else {
            bail!()
        };
        let params: WriteParams = ctx.parameter_cursor().get()?;
        ensure!(params.offset + (params.data.len() as u32) <= MAX_CONTRACT_STATE_SIZE);
        state.seek(SeekFrom::Start(params.offset.into()))?;
        state.write(&params.data)?;
        Ok(A::accept())
    }
}
